...
def download_butane(pid: int):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{prof.id}+{prof.name}.bu"'
        },
    )
...
def download_ignition(pid: int):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{prof.id}+{prof.name}.ign"'
        },
    )
...
async def upload_butane(pid: int, file: UploadFile = File(...)):
    ...
    if not (
        filename.endswith(".bu")
        or filename.endswith(".yaml")
        or filename.endswith(".yml")
    ):
        raise HTTPException(
            status_code=400, detail="Upload a Butane YAML file (.bu/.yaml/.yml)"
        )
